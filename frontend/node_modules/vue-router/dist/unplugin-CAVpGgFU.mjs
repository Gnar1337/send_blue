 ...
 * @license MIT
 */
import { _ as formatMultilineUnion, b as ts, c as appendExtensionListToPattern, f as joinPath, g as warn, h as throttle, i as resolveOptions, l as asRoutePath, m as mergeRouteRecordOverride, o as ESCAPED_TRAILING_SLASH_RE, p as logTree, r as mergeAllExtensions, s as ImportsMap, v as pad, y as toStringLiteral } from "./options-DHij9uO1.mjs";
...
${files.map(([key, path$1]) => `${indentStr + "  "}${toStringLiteral(key)}: ${generatePageImport(path$1, importMode, importsMap)}`).join(",\n")}
${indentStr}},`;
}
/**
...
function generateRouteRecordComponent(node, indentStr, importMode, importsMap) {
	...
	return `components: {
${Array.from(node.value.components).map(([key, path$1]) => `${indentStr + "  "}${toStringLiteral(key)}: ${generatePageImport(path$1, importMode, importsMap)}`).join(",\n")}
${indentStr}},`;
}
/**
