...
//#endregion
const require_options = require('./options-DCEQBG2O.cjs');
...
${files.map(([key, path$1]) => `${indentStr + "  "}${require_options.toStringLiteral(key)}: ${generatePageImport(path$1, importMode, importsMap)}`).join(",\n")}
${indentStr}},`;
}
/**
...
function generateRouteRecordComponent(node, indentStr, importMode, importsMap) {
	...
	return `components: {
${Array.from(node.value.components).map(([key, path$1]) => `${indentStr + "  "}${require_options.toStringLiteral(key)}: ${generatePageImport(path$1, importMode, importsMap)}`).join(",\n")}
${indentStr}},`;
}
/**
